...
class SMF_Diagnostics {
    ...
    private static function cron_detail($hook){
        if(defined('DISABLE_WP_CRON')&&DISABLE_WP_CRON)return array(false,'WP-Cron is disabled; verify a real server cron invokes wp-cron.php.');
        $next=wp_next_scheduled($hook);if($next===false)return array(false,'Not scheduled.');
        $delta=$next-current_time('timestamp');if($delta<0)return array(false,'Scheduled but overdue by '.absint($delta).' seconds.','warning');
        return array(true,'Scheduled · next run in '.absint($delta).' seconds.');
    }
    public static function checks(){
        global $wpdb;$checks=array();$wc=class_exists('WooCommerce');$wc_version=defined('WC_VERSION')?WC_VERSION:($wc?'installed':'inactive');
        ...
        $q=class_exists('SMF_Meta_CAPI')?SMF_Meta_CAPI::get_queue_stats():array('pending'=>0,'processing'=>0,'sent'=>0,'failed'=>0,'last_sent'=>null,'last_error'=>null);$checks[]=self::check('CAPI queue failures',(int)$q['failed']===0,$q['failed']?'Failed events: '.$q['failed'].' · review queue diagnostics':'No failed CAPI events.','warning');
        foreach(array('smf_process_capi_queue'=>'WP-Cron CAPI','smf_sync_meta_spend'=>'WP-Cron Meta spend') as $hook=>$label){$c=self::cron_detail($hook);$checks[]=self::check($label,$c[0],$c[1],isset($c[2])?$c[2]:'warning');}
